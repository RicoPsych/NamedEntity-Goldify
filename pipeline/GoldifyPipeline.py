...
class GoldifyPipeline:
    def __init__(self, pipeline: list[Metrics] = [0,1,3,4,5,6,7], consistency_chunks = 10, consistency_lang:Literal["en","fr","es","de"] = "en", user_agent = ""):
        self.pipeline = pipeline
        self.consistency_chunks = consistency_chunks
        self.consistency_lang = consistency_lang
        self.user_agent = user_agent

    def match_metric(self, metric, dataset:Dataset, result_path):
        ...
        match metric:
            ...
            case Metrics.DispersityAnnot:
                ...
                SaveResult(annot_dispersity, result_path ,"dispersity_annotations")

            case Metrics.DispersityCtx:          
                context_dispersity = ContextDispersity(dataset)
                SaveResult(context_dispersity, result_path ,"dispersity_contexts")
            ...
            case Metrics.Coherence:
                entity_coherence = EntityCoherence(dataset, user_agent = self.user_agent) #TODO:change to template later
    ...
    def assess_datasets(self, datasets: list[Dataset], result_path = "./results"):
        if type(datasets) is Dataset:
            datasets = [datasets]
        
        for metric in self.pipeline:
            for dataset in datasets:
                ds_result_path = Path(result_path) / dataset.name
                if not ds_result_path.exists():
                    ds_result_path.mkdir(exist_ok=True)
                
                self.match_metric(metric, dataset, ds_result_path)
    ...
    def assess_dataset(self, dataset: Dataset, result_path = "./results"):
        ...
        for metric in self.pipeline:
            self.match_metric(metric, dataset, result_path)  
